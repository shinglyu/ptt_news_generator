...
(function () {
        ...
        bottomfloating: function () {
            ...
            if(curTop != targetTop) {
                ...
                if(this.align === 'center') {
                    var ele = this;
                    if ($('#topMenu').length > 0) {
                        ele.ctn.style.left = $('#topMenu').offset().left + 'px';
                        $(window).resize(function() {
                            ele.ctn.style.left = $('#topMenu').offset().left + 'px';
                        })
                    } else {
                        ele.ctn.style.left = this.getCenterLeft() + 'px';
                    }
                }
            }
            this.floatTimer = setTimeout(function () {
    ...
    AD.dropDown = {
        ...
        add: function (sects, title, color, url, row) {
            ...
            var r = (row || 0);
            var header = (typeof (headervar) != 'undefined') ? headervar : '';
            bgColor = colors[0];
        ...
        addWithHeader: function (sects, title, color, url, header, row) {
            var bgColor = '';
            var txtColor = '#000';
            var s = sects.split(',');
            var colors = color.split('|');
